...
- 2026-09-14 (Grok chat): /size height bins now multiply the slot prior (theory-model.js). 7-3+ is 17.6% HOF raw (n=17, +12.9 pp vs ~4.7% base); the model shrinks that ~3.7× lift to 1.80× so 17 players cannot outrank the pick. Always emit a Height step, including “missing”. 2026 combine joined for the class (Mara 7-3 / 260 / 7-6 / 9-9, age 21.2). 2026 player cards show How-we-got-here projections + Why ledger, not career 0s. 2026 board defaults to When drafted. Do not paint slot-prior years as the hero on completed careers (1947–2025).
